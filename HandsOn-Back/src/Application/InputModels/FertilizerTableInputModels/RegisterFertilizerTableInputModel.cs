...
{
    public class RegisterFertilizerTableInputModel
    {
        [Required(ErrorMessage = "Expected bases saturation is required.")]
        ...
        public RegisterLeafFertilizerRowInputModel LeafFertilizerRow { get; set; } = new();

        [Required(ErrorMessage = "Soil fertilizer rows are required.")]
        public ICollection<RegisterSoilFertilizerRowInputModel> SoilFertilizerRows { get; set; } = [];
        ...
        public FertilizerTable ToEntity()
        {
            var tableData = new FertilizerTableData
            {
                ...
                SoilFertilizerRows = SoilFertilizerRows.Select(r => r.ToEntity()).ToList()
            };
    ...
    public class RegisterLeafFertilizerRowInputModel
    {
        [Required(ErrorMessage = "Leaf Columns are required.")]
        public ICollection<RegisterLeafFertilizerColumnInputModel> LeafFertilizerColumns { get; set; } = [];
    ...
    public class RegisterSoilFertilizerRowInputModel
    {
    ...
    public class RegisterLeafFertilizerColumnInputModel
    {
        [Required(ErrorMessage = "Header is required.")]
        public NutrientHeader Header { get; set; }

        [Required(ErrorMessage = "Products are required.")]
        public ICollection<RegisterLeafFertilizerProductInputModel> Products { get; set; } = [];
    ...
    public class RegisterLeafFertilizerProductInputModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (MinConcentration >= MaxConcentration)
            {
                yield return new ValidationResult
                    (
                        "Max concentration must be greater than Min concentration",
                        new[] { nameof(MaxConcentration), nameof(MinConcentration) }
                    );
            }
        }
    }

    public class RegisterSoilFertilizerColumnInputModel : IValidatableObject
    {
        [Required(ErrorMessage = "Header is required.")]
        public NutrientHeader Header { get; set; }

        [Required(ErrorMessage = "Number of values is required.")]
        [Range(2, 4, ErrorMessage = "Number of thresholds must be 2, 3 or 4.")]
        public int NumberOfValues { get; set; }

        [Required(ErrorMessage = "Value 1 is required.")]
        [Range(0, float.MaxValue, ErrorMessage = "Value 1 cannot be negative.")]
