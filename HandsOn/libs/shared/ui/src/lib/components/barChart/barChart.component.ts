...
export class BarChartComponent implements OnInit, OnChanges {
  ...
  get value(): FormControl {
    return this.barForm.get('value') as FormControl;
  }

  ngOnChanges(changes: SimpleChanges) {
    if (changes['analysisData'] && this.barForm) {
      const newValue = changes['analysisData'].currentValue.value;
      this.value.setValue(newValue, { emitEvent: false });
    }
  }

  ngOnInit() {
    this.value.setValue(this.analysisData.value);

    this.barForm.valueChanges.subscribe(formValues => {
      if (this.analysisData && this.barForm.valid) {
  ...
  public getDisplayLevel(): string {
    ...
    if (this.analysisData.isSimpleRange) {
      const labels: Record<SimpleClassification, string> = {
        'low': 'Baixo',
        'medium': 'Aceitável',
        'adequate': 'Adequado',
        'high': 'Alto'
      ...
      if (this.analysisData.inverted) {
        const labels: Record<ComplexClassification, string> = {
          'very-low': 'Muito Bom',
          'low': 'Bom',
          'medium': 'Médio',
          'good': 'Alto',
          'very-good': 'Muito Alto'
        };
